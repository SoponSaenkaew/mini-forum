...
const PostCard = ({ post }) => (
    <article className="text-left rounded-3xl border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 bg-white overflow-hidden flex flex-col group" aria-label={`โพสต์โดย ${post.user.name}`}>
        ...
        <div className="p-5 flex flex-col flex-1">
            ...
            <p className="text-gray-700 line-clamp-2 mb-4 text-sm leading-relaxed flex-1">
                {post.content}
            </p>
 ...
 * ประกอบด้วยส่วนต้อนรับ (Hero Section) และการแสดงผลโพสต์ล่าสุด (Trending Activity)
 */
export default function Welcome({ auth, latestPosts }) {
    return (
        <div className="min-h-screen bg-white text-gray-900 selection:bg-indigo-500 selection:text-white flex flex-col">
            ...
            {/* แถบนำทางส่วนหัว (Top Navigation Bar) */}
            ...
            <main className="max-w-7xl mx-auto px-6 flex-1 flex flex-col items-center">
                ...
                {/* ส่วนแนะนำและเชิญชวน (Hero Section) */}
                ...
                {/* ส่วนแสดงผลกิจกรรมล่าสุด (Trending Activity Feed) */}
                <section className="mt-8 mb-20 w-full" aria-labelledby="trending-heading">
                    ...
                    {/* ตารางแสดงการ์ดโพสต์ (Grid Layout) */}
